...
CREATE USER IF NOT EXISTS mcsuser WITH
    ...
    GRANT ALL PRIVILEGES ON DATABASE mcsunity TO mcsuser;
...
CREATE TABLE token (
    ...
    roles VARCHAR(50) NOT NULL DEFAULT 'API',
...
CREATE TABLE useraccount (
    ...
    database_id int NOT NULL,
    active BOOLEAN DEFAULT '0' NOT NULL,
    roles VARCHAR(50) NOT NULL DEFAULT 'Not Defined',
    FOREIGN KEY (database_id) REFERENCES database_list(database_id)
);

insert into useraccount (username,password,database_id,active,roles) values('nasar','nasar165',1,'1', 'Admin');
